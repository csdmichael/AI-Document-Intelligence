...
The portal supports two distinct **use cases**, selectable from the dropdown in the top navigation bar. Switching use cases instantly re-filters all documents on the dashboard so reviewers see only the documents relevant to their workflow.

### How to Switch

In the top navigation bar, select a use case from the **Use Case** dropdown:

- **📋 Tax Forms** — shows handwritten-style tax exemption PDF documents
- **🔧 Eng Docs** — shows engineering PowerPoint (PPTX) documents
...
|---------|--------|
| **Document type** | PDF |
| **Source** | Generated by `scripts/generate_forms.py` |
| **Filter options** | Confidence category, US State, Document Type, Review status |
| **State filter** | A **State** dropdown appears exclusively for this use case, letting reviewers narrow results to a single US state |
| **Typical fields** | Company Name, Tax ID, Exemption Reason, Authorized Signature, etc. |

#### 🔧 Eng Docs

Processes **PPTX** (PowerPoint) engineering documents such as technical specifications, design presentations, or project decks.

| Feature | Detail |
|---------|--------|
| **Document type** | PPTX |
| **Filter options** | Confidence category, Document Type, Review status |
| **State filter** | Not applicable — hidden for this use case |
| **Typical content** | Slide text, embedded diagrams, images, and table data extracted by AI Document Intelligence |
| **Viewer** | PPTX files render inline via the Office Online viewer (with Google Docs viewer as fallback) |

### Filter Behavior per Use Case

| Filter | Tax Forms | Eng Docs |
|--------|-----------|----------|
| Confidence category tabs (Blue / Green / Yellow / Red) | ✅ | ✅ |
| State dropdown | ✅ | ❌ (hidden) |
| Document Type dropdown | ✅ | ✅ |
| Review status tabs (All / Reviewed / Not Reviewed) | ✅ | ✅ |
